...
// Updated function to check if a cottage is available for given dates
...
// Updated function to get all cottages with availability status for given dates
export const getAvailableCottages = async (checkIn, checkOut) => {
  const query = `
    SELECT c.cottage_id, c.name, 
      CASE WHEN EXISTS (
...
export const createReservation = async (data) => {
  ...
  const values = [full_name, email, phone, cottage_id || null, check_in, check_out, guests, special_requests];
...
// For backward compatibility - rename extractAllData to getAllReservations
